...
export default function StepPhotoModal({
  tripId, stepId, stepName, dayIndex, onClose, onCountChange,
}: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getStepPhotos(tripId, stepId);
        if (!cancelled) {
          setPhotos(data);
          onCountChange?.(data.length);
        }
      } catch (e) {
        console.warn('[step-photos] fetch failed', e);
      } finally {
  ...
  const handleUploadConfirm = async () => {
    ...
    try {
      ...
      console.error('[step-photos] upload failed', e);
  ...
  const handleToggleCover = async (photo: TripPhoto) => {
    try {
      if (photo.is_cover) {
        ...
        toast.info(t('step_photo_cover_set'));
      }
    } catch (e: any) {
      toast.error(t('step_photo_cover_error'));
    }
  };
  ...
  const handleCaptionEdit = async (photo: TripPhoto, newCaption: string) => {
    try {
      ...
      toast.error(t('step_photo_caption_error'));
    }
  };

  const handleDelete = async (photo: TripPhoto) => {
    if (!confirm(t('step_photo_delete_confirm'))) return;
    try {
      ...
      toast.error(t('step_photo_delete_error'));
    }
  };
      ...
      >
        <div style={{
          ...
          <p style={{
            ...
            {photos.length} {photos.length > 1 ? t('step_photo_count_plural') : t('step_photo_count')}
          </p>
        </div>
        ...
        {loading ? (
          ...
          >
            <ImageIcon size={32} color="var(--accent)" strokeWidth={1.6} />
            <p style={{
              fontFamily: 'var(--f-display)', fontStyle: 'italic',
              fontSize: 17, color: 'var(--text)', margin: 0, letterSpacing: '-0.01em',
            }}>
              {t('step_photo_capture_1')} <span style={{ color: 'var(--accent)' }}>{t('step_photo_capture_2')}</span>
            </p>
            <p style={{
              fontSize: 12, color: 'var(--text-muted)', margin: 0, textAlign: 'center',
            }}>
              {t('step_photo_empty_desc')}
            </p>
          </button>
        ) : (
          <div style={{
              ...
              >
                ...
                {photo.is_cover && (
                  <div style={{
                    ...
                    <Star size={12} color="#1A1208" strokeWidth={2.5} fill="#1A1208" />
                  </div>
                )}
                {photo.caption && (
                  <div style={{
                    ...
                    fontFamily: 'var(--f-body)', fontSize: 12, color: '#fff',
                    fontWeight: 500,
      ...
      >
        {editing ? (
          <input
            ...
            maxLength={120}
            placeholder={t('step_photo_add_caption')}
            style={{
              width: '100%', padding: '12px 14px',
          ...
          >
            {photo.caption || t('step_photo_add_caption_placeholder')}
          </button>
        )}
      </div>
    </div>
